...
image_collection_l1c = "COPERNICUS/S2"
image_collection_l2a = "COPERNICUS/S2_SR"
...
# have them input as strings, damn it
def get_single_image_s22a(region: ee.Geometry, start_date, end_date, image_collection : ee.ImageCollection):

    '''
    cloud score is already calculated here
    '''
    dates = ee.DateRange(
        start_date
        end_date,
    ...
    imgC = (
        ...
        #.map(maskS2clouds) # input by Sheila
        .map(computeS2CloudScore_l2a)#replace this with calculated cloud probability thing
        .map(projectShadows) # replace this with calculated cloud shadow
...
def computeS2CloudScore_l2a(image):
  # from https://gis.stackexchange.com/questions/412948/google-earth-engine-updatemask-removes-metadata
    qa = image.select('QA60')
    slc = image.select('SLC')
    cld_prob = image.select('MSK_CLDPRB')
    score = image.select('MSK_CLDPRB').divide(100) #would this be equivalent to the cloud score generated?
    return image.addBands(score.rename("cloudScore"))



#####
# Original Sentinel-2 processing from cropharvest, applied to S2 L1C
###

def get_single_image_choice(region: ee.Geometry, start_date, end_date, image_collection : ee.ImageCollection) -> ee.Image:
